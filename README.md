...
3. **Cấu hình biến môi trường:**
   - Sao chép file `.env.example` thành `.env`:
     ...
     cp .env.example .env
     ```
   - Mở file `.env` và điền các giá trị của bạn. Hướng dẫn lấy các thông số:
     - **Firebase (Dùng cho Chat Hub):**
       1. Truy cập [Firebase Console](https://console.firebase.google.com/).
       2. Tạo một dự án mới (hoặc dùng dự án có sẵn).
       3. Thêm một ứng dụng Web (Web App) vào dự án.
       4. Firebase sẽ cung cấp một đoạn mã cấu hình chứa `apiKey`, `authDomain`, `projectId`, v.v. Hãy copy các giá trị đó vào các biến tương ứng trong file `.env`.
     - **Gemini API Key (Dùng cho AI Chatbot):**
       1. Truy cập [Google AI Studio](https://aistudio.google.com/).
       2. Đăng nhập và tạo một API Key mới.
       3. Copy API Key đó vào biến `REACT_APP_GEMINI_API_KEY` trong file `.env`.
