...
import { allWebArchiveProjects as allWebProjects } from "../data/portfolioData";

export default function WebArchive({ onBack, onSelectProject }) {
    const [search, setSearch] = useState("");
    const [filter, setFilter] = useState("All");

    const filteredProjects = allWebProjects.filter((p) => {
    ...
    return (
        <div className="min-h-screen bg-[#0a0c10] text-white py-12 px-6 md:px-12 relative z-50">
            ...
            <div className="max-w-7xl mx-auto flex items-center justify-between border-b border-white/10 pb-6 mb-12">
                <button
                    onClick={onBack}
                    className="flex items-center gap-2 font-mono text-xs uppercase tracking-wider text-acid hover:translate-x-[-4px] transition-transform py-2 px-4 rounded-xl bg-white/5 border border-white/10 hover:border-acid/40"
                >
                    ← Quay lại trang chủ
                </button>
                <span className="font-mono text-xs text-white/40 uppercase tracking-widest hidden md:inline">
                    WEB_APPLICATIONS_ARCHIVE // 2023 - 2026
                </span>
            </div>
            ...
            <div className="max-w-7xl mx-auto mb-12 text-left">
                <div className="flex items-center gap-3 mb-2">
                    <span className="w-3 h-3 rounded-full bg-acid animate-pulse" />
                    <span className="font-mono text-xs text-acid tracking-[0.2em] uppercase">
                        KHO LƯU TRỮ WEB CHUYÊN SÂU
                    </span>
                </div>
                <h1 className="font-display uppercase text-4xl md:text-6xl font-bold text-white leading-none mb-6">
                    Web Archive<span className="text-acid">.</span>
                </h1>
                ...
                <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between bg-[#0f1115] p-4 rounded-2xl border border-white/10">
                    <input
                        type="text"
                        placeholder="Tìm kiếm theo tên, mô tả hoặc công nghệ (ví dụ: React, FastAPI)..."
                        value={search}
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
                {filteredProjects.map((p) => (
                    <div
                        ...
                        className="h-[260px] rounded-2xl border border-white/10 p-6 bg-[#0f1115] hover:border-acid hover:scale-[1.02] transition-all duration-300 flex flex-col justify-between relative group overflow-hidden cursor-pointer shadow-xl text-left"
                    >
                        {/* Ảnh nền overlay */}
                        ...
                        <div className="relative z-20">
                            <div className="flex items-center justify-between mb-3">
                                <span className="font-mono text-acid text-xs font-bold tracking-widest">
                                    {p.n}
                                </span>
                            ...
                            <h3 className="font-display uppercase text-2xl font-bold text-white group-hover:text-acid transition-colors">
                                {p.name}
                            </h3>
                        ...
                        <div className="pt-3 border-t border-white/10 flex flex-col gap-2 relative z-20">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="font-mono text-[10px] font-bold text-acid uppercase tracking-wider line-clamp-1">
                                    {p.metrics}
                                </span>
                            </div>
